...
const div = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://192.168.2.9:4001/divisi');
  ...
  const filteredData = data.filter(
    (item) =>
      ...
      item.lokasis?.nama.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleDelete = (id) => {
    SweetAlert.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://192.168.2.9:4001/divisi/${id}`)
          .then((res) => {
  ...
  return (
    <CCard className="mb-4">
      ...
      <CCardBody>
        ...
        <CRow className="mb-3">
          <CCol>
            <Link to="/div/add" className="btn btn-primary">
          ...
          <CTableBody>
            {currentData.map((item, index) => (
              <CTableRow key={index}>
                ...
                <CTableDataCell>{item.lokasis?.nama}</CTableDataCell>
                <CTableDataCell>
                  {/* Edit and Delete buttons */}
                  <Link to={`/div/edit/${item.id}`}>
